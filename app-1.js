...
const pageTitles={dashboard:"ภาพรวม",calendar:"รายการต้องจ่าย",debts:"หนี้ทั้งหมด",products:"ผ่อนสินค้า",income:"เงินเข้า",rotation:"เงินหมุน",forecast:"แผนล่วงหน้า"};
const pageBadges={dashboard:"แดชบอร์ด",calendar:"ปฏิทินรายจ่าย",debts:"ภาพรวมหนี้",products:"ตารางผ่อนสินค้า",income:"บันทึกรายรับ",rotation:"วางแผนเงินหมุน",forecast:"คาดการณ์ 6 เดือน"};
...
byId("moreProductsBtn")?.addEventListener("click", ()=>{ closeMore(); navigateTo("products"); });
...
document.querySelectorAll(".add-type").forEach(btn=>btn.onclick=()=>{
  if(btn.dataset.type==="product_installment"){closeModal();openProductInstallmentModal();return;}
  currentType=btn.dataset.type;setActiveType();buildForm();
});
...
function smartDebtFields(type,today){
  ...
  if(type==="loan"){
    return `
      ${field("name","ชื่อสินเชื่อ","text",'required placeholder="เช่น SEasyCash หรือสินเชื่อธนาคาร"')}
      ${field("totalDebt","ยอดหนี้คงเหลือปัจจุบัน","number",'min="0" step="0.01" required')}
      ...
      ${selectField("seasycashAccount","บัญชีนี้จะมีกดเงินเพิ่มเป็นรอบหรือไม่",[["no","ไม่ — สินเชื่อทั่วไป"],["yes","ใช่ — SEasyCash / เพิ่มรอบกู้ได้"]])}
      ${selectField("payer","ผู้จ่ายเจ้าหนี้",[["me","ฉัน"],["partner","แฟน"],["other","คนอื่น"]])}
      <div class="field full debt-form-note">
        ถ้าเป็น SEasyCash ที่มีหนี้เดิมอยู่แล้ว ให้กรอกเฉพาะยอดคงเหลือปัจจุบัน แล้วเลือก “SEasyCash / เพิ่มรอบกู้ได้”
        จากนั้นรอบใหม่ในอนาคตค่อยกด “เพิ่มรอบกู้ใหม่” ที่การ์ดหนี้เดิม
      </div>`;
  }
...
function saveCurrent(closeAfter=true){
  ...
  if(currentType==="debt"){
    ...
    if(t==="loan"){
      const total=num(d.totalDebt), bill=num(d.currentBill);
      const asRounds=d.seasycashAccount==="yes";
      state.debts.push({
        ...
        payer:d.payer,createdAt:todayKey(),loanRoundMode:asRounds,
        loanRounds:asRounds?[]:undefined,
        openingLoanBalance:asRounds?total:undefined,
        openingLoanRemaining:asRounds?total:undefined
      });
      state.payments.push({
        id:uid("pay"),debtId,name:d.name,amount:bill,myShare:bill,partnerShare:0,partnerReceived:0,
        dueDate:d.dueDate,paid:false,type:"debt",payer:d.payer,debtKind:"loan",debtReduction:null,
        openingLoanPayment:asRounds
      });
    }
...
let editingProductInstallmentId=null;
function productMonthLabel(ym){
  if(!ym)return "-";
  const [y,m]=ym.split("-").map(Number);
  return new Date(y,m-1,1).toLocaleDateString("th-TH",{month:"long",year:"numeric"});
}
function productMonthPlus(ym,add){
  const [y,m]=ym.split("-").map(Number), d=new Date(y,m-1+add,1);
  return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`;
}
function productPayerLabel(mode,name="แฟน"){return mode==="me"?"เรา":mode==="partner"?(name||"แฟน"):`เรา + ${name||"แฟน"}`;}
function productSummaryPaymentId(ym){return `product_summary_${ym.replace("-","_")}`;}
function allProductScheduleRows(){
  const rows=[];
  (state.productInstallments||[]).forEach(plan=>(plan.schedule||[]).forEach(r=>rows.push({...r,planId:plan.id,productName:plan.name,payerMode:plan.payerMode||"me",partnerName:plan.partnerName||"แฟน"})));
  return rows;
}
function productMonthStats(ym){
  const rows=allProductScheduleRows().filter(r=>r.month===ym);
  const s={rows,total:0,myTotal:0,partnerTotal:0,myOnlyCount:0,myOnlyAmount:0,partnerOnlyCount:0,partnerOnlyAmount:0,sharedCount:0,sharedTotal:0,sharedMine:0,sharedPartner:0};
  rows.forEach(r=>{
    s.total+=num(r.amount);s.myTotal+=num(r.myShare);s.partnerTotal+=num(r.partnerShare);
    if(r.payerMode==="me"){s.myOnlyCount++;s.myOnlyAmount+=num(r.amount);}
    else if(r.payerMode==="partner"){s.partnerOnlyCount++;s.partnerOnlyAmount+=num(r.amount);}
    else{s.sharedCount++;s.sharedTotal+=num(r.amount);s.sharedMine+=num(r.myShare);s.sharedPartner+=num(r.partnerShare);}
  });
  return s;
}
function syncProductInstallmentSummaryPayments(){
  if(!Array.isArray(state.productInstallments))state.productInstallments=[];
  const grouped={};
  allProductScheduleRows().forEach(r=>{
    if(!r.month)return;
    grouped[r.month] ||= {total:0,my:0,partner:0,count:0,due:[]};
    grouped[r.month].total+=num(r.amount);grouped[r.month].my+=num(r.myShare);grouped[r.month].partner+=num(r.partnerShare);grouped[r.month].count++;
    if(r.dueDate)grouped[r.month].due.push(r.dueDate);
  });
  const keep=new Set();
  Object.entries(grouped).forEach(([ym,g])=>{
    const id=productSummaryPaymentId(ym);keep.add(id);
    let p=state.payments.find(x=>x.id===id);
    const due=g.due.sort()[0]||`${ym}-01`;
    if(!p){p={id,paid:false,paidDate:null,type:"product_installment_summary",payer:"mixed",partnerReceived:0};state.payments.push(p);}
    Object.assign(p,{name:`ผ่อนสินค้า • ${productMonthLabel(ym)}`,amount:g.total,myShare:g.my,partnerShare:g.partner,dueDate:due,productMonth:ym,productItemCount:g.count});
  });
  state.payments=state.payments.filter(p=>p.type!=="product_installment_summary"||keep.has(p.id));
}
function cashOutAmount(p){
  if(p?.type==="product_installment_summary")return num(p.myShare);
  return p?.payer==="me"?num(p.amount):0;
}
function openProductInstallmentModal(id=null){
  editingProductInstallmentId=id;
  const plan=id?(state.productInstallments||[]).find(x=>x.id===id):null;
  byId("productModalTitle").textContent=plan?"แก้ไขรายการผ่อนสินค้า":"เพิ่มรายการผ่อนสินค้า";
  byId("productInstallmentName").value=plan?.name||"";
  byId("productPayerMode").value=plan?.payerMode||"me";
  byId("productPartnerName").value=plan?.partnerName||"แฟน";
  byId("productInstallmentCount").value=plan?.schedule?.length||1;
  byId("productFirstMonth").value=plan?.schedule?.[0]?.month||activeMonthKey();
  renderProductInstallmentEditor(plan?.schedule||[]);
  byId("productInstallmentModal").classList.remove("hidden");
}
window.openProductInstallmentModal=openProductInstallmentModal;
function closeProductInstallmentModal(){byId("productInstallmentModal")?.classList.add("hidden");editingProductInstallmentId=null;}
function renderProductInstallmentEditor(existing=null){
  const box=byId("productInstallmentRows");if(!box)return;
  const mode=byId("productPayerMode").value;
  const count=Math.max(1,parseInt(byId("productInstallmentCount").value||"1",10));
  const first=byId("productFirstMonth").value||activeMonthKey();
  let old=Array.isArray(existing)&&existing.length?existing:[...box.querySelectorAll(".product-editor-row")].map(row=>({
    id:row.dataset.rowId,month:row.querySelector('[data-prod="month"]')?.value,dueDate:row.querySelector('[data-prod="due"]')?.value,
    amount:num(row.querySelector('[data-prod="amount"]')?.value),myShare:num(row.querySelector('[data-prod="mine"]')?.value),partnerShare:num(row.querySelector('[data-prod="partner"]')?.value)
  }));
  box.innerHTML=Array.from({length:count},(_,i)=>{
    const month=old[i]?.month||productMonthPlus(first,i);
    const due=old[i]?.dueDate||`${month}-28`;
    const id=old[i]?.id||uid("prodrow");
    const fields=mode==="shared"
      ? `<label>ส่วนเรา<input data-prod="mine" type="number" min="0" step="0.01" value="${num(old[i]?.myShare)||""}"></label><label>ส่วนแฟน<input data-prod="partner" type="number" min="0" step="0.01" value="${num(old[i]?.partnerShare)||""}"></label><div class="product-row-total"><span>รวม</span><strong data-prod-total>${money(num(old[i]?.myShare)+num(old[i]?.partnerShare))}</strong></div>`
      : `<label>ยอดงวดนี้<input data-prod="amount" type="number" min="0" step="0.01" value="${num(old[i]?.amount)||""}" required></label>`;
    return `<div class="product-editor-row" data-row-id="${id}"><div class="product-row-no">งวด ${i+1}</div><label>เดือน<input data-prod="month" type="month" value="${month}" required></label><label>ครบกำหนด<input data-prod="due" type="date" value="${due}" required></label>${fields}</div>`;
  }).join("");
  byId("productEditorHint").textContent=mode==="shared"?"กรอกส่วนเราและแฟนแยกกัน":mode==="partner"?"แฟนเป็นผู้จ่าย":"เราเป็นผู้จ่าย";
  box.querySelectorAll('[data-prod="mine"],[data-prod="partner"]').forEach(inp=>inp.addEventListener("input",()=>{
    const row=inp.closest(".product-editor-row"), el=row.querySelector("[data-prod-total]");
    if(el)el.textContent=money(num(row.querySelector('[data-prod="mine"]')?.value)+num(row.querySelector('[data-prod="partner"]')?.value));
  }));
}
function collectProductSchedule(){
  const mode=byId("productPayerMode").value;
  return [...document.querySelectorAll("#productInstallmentRows .product-editor-row")].map((row,i)=>{
    let amount=0,myShare=0,partnerShare=0;
    if(mode==="shared"){myShare=num(row.querySelector('[data-prod="mine"]').value);partnerShare=num(row.querySelector('[data-prod="partner"]').value);amount=myShare+partnerShare;}
    else{amount=num(row.querySelector('[data-prod="amount"]').value);if(mode==="me")myShare=amount;else partnerShare=amount;}
    return {id:row.dataset.rowId||uid("prodrow"),no:i+1,month:row.querySelector('[data-prod="month"]').value,dueDate:row.querySelector('[data-prod="due"]').value,amount,myShare,partnerShare};
  });
}
function saveProductInstallment(){
  if(!byId("productInstallmentForm").reportValidity())return;
  const name=byId("productInstallmentName").value.trim(), payerMode=byId("productPayerMode").value, partnerName=byId("productPartnerName").value.trim()||"แฟน", schedule=collectProductSchedule();
  if(schedule.some(r=>r.amount<=0))return alert("กรุณากรอกยอดทุกงวด");
  const wasEditing=!!editingProductInstallmentId;
  if(wasEditing){const p=state.productInstallments.find(x=>x.id===editingProductInstallmentId);Object.assign(p,{name,payerMode,partnerName,schedule});}
  else state.productInstallments.push({id:uid("product"),name,payerMode,partnerName,schedule,createdAt:todayKey()});
  syncProductInstallmentSummaryPayments();saveState();renderAll();closeProductInstallmentModal();navigateTo("products");toast(wasEditing?"แก้ไขรายการผ่อนแล้ว":"เพิ่มรายการผ่อนสินค้าแล้ว");
}
window.editProductInstallment=id=>openProductInstallmentModal(id);
window.deleteProductInstallment=function(id){
  const p=state.productInstallments.find(x=>x.id===id);if(!p)return;
  if(!confirm(`ลบ "${p.name}" ใช่ไหม?`))return;
  state.productInstallments=state.productInstallments.filter(x=>x.id!==id);syncProductInstallmentSummaryPayments();saveState();renderAll();toast("ลบรายการผ่อนแล้ว");
};
function renderProductMonthOptions(){
  const s=byId("productMonthFilter");if(!s)return;
  const old=s.value, months=new Set(allProductScheduleRows().map(r=>r.month).filter(Boolean));
  for(let i=-1;i<12;i++)months.add(productMonthPlus(activeMonthKey(),i));
  const arr=[...months].sort();s.innerHTML=arr.map(x=>`<option value="${x}">${productMonthLabel(x)}</option>`).join("");s.value=arr.includes(old)?old:activeMonthKey();
}
function renderProductInstallments(){
  if(!byId("products"))return;
  renderProductMonthOptions();
  const ym=byId("productMonthFilter")?.value||activeMonthKey(), st=productMonthStats(ym);
  byId("productMyOnlyAmount").textContent=money(st.myOnlyAmount);byId("productMyOnlyCount").textContent=st.myOnlyCount;
  byId("productPartnerOnlyAmount").textContent=money(st.partnerOnlyAmount);byId("productPartnerOnlyCount").textContent=st.partnerOnlyCount;
  byId("productSharedTotal").textContent=money(st.sharedTotal);byId("productSharedCount").textContent=st.sharedCount;byId("productSharedMine").textContent=money(st.sharedMine);byId("productSharedPartner").textContent=money(st.sharedPartner);
  byId("productGrandTotal").textContent=money(st.total);byId("productGrandMine").textContent=money(st.myTotal);byId("productGrandPartner").textContent=money(st.partnerTotal);byId("productGrandCount").textContent=`${st.rows.length} รายการ`;
  const rows=[...st.rows].sort((a,b)=>a.dueDate.localeCompare(b.dueDate));
  byId("productMonthlyTable").innerHTML=rows.length?`<div class="product-table-scroll"><table><thead><tr><th>สินค้า</th><th>งวด</th><th>ครบกำหนด</th><th>ผู้จ่าย</th><th>ยอดรวม</th><th>เรา</th><th>แฟน</th></tr></thead><tbody>${rows.map(r=>`<tr><td><strong>${r.productName}</strong></td><td>${r.no}</td><td>${thaiDate(r.dueDate)}</td><td><span class="product-payer-chip ${r.payerMode}">${productPayerLabel(r.payerMode,r.partnerName)}</span></td><td><strong>${money(r.amount)}</strong></td><td>${money(r.myShare)}</td><td>${money(r.partnerShare)}</td></tr>`).join("")}</tbody></table></div>`:`<div class="empty">เดือนนี้ยังไม่มีรายการผ่อนสินค้า</div>`;
  const plans=state.productInstallments||[];
  byId("productPlanCards").innerHTML=plans.length?plans.map(p=>{
    const total=p.schedule.reduce((s,r)=>s+num(r.amount),0), mine=p.schedule.reduce((s,r)=>s+num(r.myShare),0), partner=p.schedule.reduce((s,r)=>s+num(r.partnerShare),0);
    return `<div class="product-plan-card"><div class="product-plan-card-head"><div><strong>${p.name}</strong><small>${productPayerLabel(p.payerMode,p.partnerName)} • ${p.schedule.length} งวด</small></div><span class="product-payer-chip ${p.payerMode}">${productPayerLabel(p.payerMode,p.partnerName)}</span></div><div class="product-plan-stats"><div><span>รวม</span><strong>${money(total)}</strong></div><div><span>เรา</span><strong>${money(mine)}</strong></div><div><span>แฟน</span><strong>${money(partner)}</strong></div></div><div class="product-mini-schedule">${p.schedule.map(r=>`<div><span>${productMonthLabel(r.month)} • ${thaiDate(r.dueDate)}</span><strong>${money(r.amount)}</strong></div>`).join("")}</div><div class="card-actions"><button class="edit-btn" onclick="editProductInstallment('${p.id}')">แก้ไข</button><button class="delete-btn" onclick="deleteProductInstallment('${p.id}')">ลบ</button></div></div>`;
  }).join(""):`<div class="empty">ยังไม่มีรายการผ่อนสินค้า</div>`;
}
byId("addProductInstallmentBtn")?.addEventListener("click",()=>openProductInstallmentModal());
byId("closeProductInstallmentBtn")?.addEventListener("click",closeProductInstallmentModal);
byId("cancelProductInstallmentBtn")?.addEventListener("click",closeProductInstallmentModal);
byId("saveProductInstallmentBtn")?.addEventListener("click",saveProductInstallment);
byId("productInstallmentModal")?.addEventListener("click",e=>{if(e.target===byId("productInstallmentModal"))closeProductInstallmentModal();});
byId("productPayerMode")?.addEventListener("change",()=>renderProductInstallmentEditor());
byId("productInstallmentCount")?.addEventListener("change",()=>renderProductInstallmentEditor());
byId("productFirstMonth")?.addEventListener("change",()=>renderProductInstallmentEditor());
byId("productMonthFilter")?.addEventListener("change",renderProductInstallments);
...
function getCurrentBalance(asOf=planningDateKey()){
  ...
  const paid=state.payments
    .filter(x=>x.paid && cashOutAmount(x)>0 && (!start || !x.paidDate || x.paidDate>=start) && (!x.paidDate || x.paidDate<=asOf))
    .reduce((s,x)=>s+cashOutAmount(x),0);
  return num(state.balanceAdjustment)+income-paid;
}
function effectiveMyBurden(p){if(p.type==="product_installment_summary")return num(p.myShare);return (p.type==="shared"||p.type==="shared_installment")?(p.payer==="me"?num(p.myShare):0):(p.payer==="me"?num(p.amount):0);}
...
function renderDashboard(){
  ...
  const futureMyPays=monthPays.filter(p=>!p.paid&&p.dueDate>=refDate).reduce((s,p)=>s+cashOutAmount(p),0);
  ...
  monthPays.filter(p=>!p.paid&&p.dueDate>=refDate&&cashOutAmount(p)>0).forEach(p=>{
    if(expectedPartner(p)>0)timeline.push({date:p.transferDate||p.dueDate,delta:expectedPartner(p)});
    timeline.push({date:p.dueDate,delta:-cashOutAmount(p)});
  });
...
function labelType(t){return({debt:"หนี้",shared:"หนี้ร่วม",shared_installment:"ผ่อนร่วม",expense:"ค่าใช้จ่าย",rotation:"เงินหมุน",product_installment_summary:"ผ่อนสินค้า"})[t]||t;}
...
function renderPayments(){
  ...
  byId("paymentList").innerHTML=list.length?list.map(p=>`<div class="payment-row ${p.paid?"paid":""} ${p.dueDate<todayKey()&&!p.paid?"overdue":""}">
    ...
    <div><strong>${p.name}</strong><br><small>${thaiDate(p.dueDate)} ${p.type==="product_installment_summary"?`• ${p.productItemCount||0} รายการ • เรา ${money(p.myShare)} • แฟน ${money(p.partnerShare)}`:(p.type==="shared"||p.type==="shared_installment")?`• ${p.partnerName||"ผู้ร่วมจ่าย"} ${money(p.partnerShare)}${p.installmentNo?` • งวด ${p.installmentNo}/${p.totalInstallments}`:""}`:""}</small></div>
    ...
    <div style="text-align:right"><strong>${money(p.amount)}</strong><br><div class="row-actions">${(p.type==="shared"||p.type==="shared_installment")&&!p.paid?`<button class="action-link" onclick="receiveShared('${p.id}')">รับเงินร่วม</button>`:""}${p.type==="product_installment_summary"?`<button class="action-link" onclick="navigateTo('products')">ดูรายละเอียด</button>`:`<button class="mini-edit-btn" onclick="editPayment('${p.id}')"><i class="ph ph-pencil-simple"></i></button><button class="mini-delete-btn" onclick="deletePayment('${p.id}')"><i class="ph ph-trash"></i></button>`}</div></div></div>`).join(""):`<div class="empty">ไม่มีรายการในเดือนนี้</div>`;
}
...
window.togglePaid=function(id){
  ...
  if(p.debtId && !p.loanRoundId){
    ...
    if(d){
      ...
      if(p.openingLoanPayment && isSpecialLoanMode(d)){
        d.openingLoanRemaining=Math.max(0,num(d.openingLoanRemaining)+(p.paid?-reduction:reduction));
...
window.adjustDebtBalance=function(id){
  ...
  const target=Math.max(0,num(val));
  if(isSpecialLoanMode(d)){
    const roundsRemaining=(d.loanRounds||[]).reduce((sum,r)=>sum+(r.installments||[])
      .filter(i=>!i.paid).reduce((s,i)=>s+num(i.amount),0),0);
    d.openingLoanRemaining=Math.max(0,target-roundsRemaining);
    if(d.openingLoanBalance==null) d.openingLoanBalance=d.openingLoanRemaining;
    recalcDebtFromRounds(d);
  }else{
    d.remaining=target;
  }
...
window.toggleLoanRoundMode=function(id){
  const d=state.debts.find(x=>x.id===id); if(!d)return;
  const turningOn=!isSpecialLoanMode(d);
  d.loanRoundMode=turningOn;
  if(turningOn){
    if(!Array.isArray(d.loanRounds)) d.loanRounds=[];
    if(d.openingLoanBalance==null) d.openingLoanBalance=Math.max(0,num(d.remaining));
    if(d.openingLoanRemaining==null) d.openingLoanRemaining=Math.max(0,num(d.remaining));
    state.payments.filter(p=>p.debtId===d.id && !p.loanRoundId).forEach(p=>p.openingLoanPayment=true);
  }
  saveState();renderAll();
  toast(turningOn?"เปิดใช้ SEasyCash — หนี้เดิมถูกเก็บเป็นยอดตั้งต้นแล้ว":"กลับเป็นรูปแบบหนี้ปกติแล้ว");
};
...
function renderDebts(){
  byId("debtCards").innerHTML=state.debts.length?state.debts.map(d=>{
    ...
    const openingLoanHtml=special && num(d.openingLoanBalance)>0 ? `
      <div class="opening-loan-summary">
        <div><small>ยอดเดิมก่อนเริ่มใช้ระบบ</small><strong>${money(d.openingLoanRemaining)}</strong></div>
        <div><small>ยอดตั้งต้นเดิม</small><strong>${money(d.openingLoanBalance)}</strong></div>
      </div>`:"";
...
function recalcDebtFromRounds(debt){
  if(!debt) return;
  const opening=Math.max(0,num(debt.openingLoanRemaining));
  let roundsRemaining=0;
  (debt.loanRounds||[]).forEach(r=>{
    (r.installments||[]).forEach(i=>{
      if(!i.paid) roundsRemaining+=num(i.amount);
    });
  });
  debt.remaining=opening+roundsRemaining;

  const originalOpening=Math.max(0,num(debt.openingLoanBalance));
  const roundsTotal=(debt.loanRounds||[]).reduce((s,r)=>s+num(r.totalRepayment),0);
  if(originalOpening || roundsTotal) debt.totalDebt=originalOpening+roundsTotal;

  const nextPay=state.payments
    .filter(p=>p.debtId===debt.id && !p.paid)
    .sort((a,b)=>(a.dueDate||"9999").localeCompare(b.dueDate||"9999"))[0];
  debt.currentBill=num(nextPay?.amount);
  debt.monthlyAmount=debt.currentBill;
}
...
function openLoanRoundModal(debtId){
  ...
  const borrowedRaw=prompt("ยอดเงินที่กด / ได้รับจริงรอบใหม่","0");
  if(borrowedRaw===null)return;
  const borrowed=Math.max(0,num(borrowedRaw));
  if(borrowed<=0)return toast("กรุณาใส่ยอดเงินที่กด");

  const startDate=prompt("วันที่ได้รับเงิน (YYYY-MM-DD)",todayKey());
  if(startDate===null)return;

  const countRaw=prompt("เลือกผ่อนกี่เดือน","2");
  ...
  const round={
    id:uid("round"),startDate,borrowedAmount:borrowed,totalInterest:0,totalFee:0,
    totalRepayment:0,installmentCount:count,moneyReceived:false,incomeId:null,installments:[]
  };

  for(let i=0;i<count;i++){
    const amt=prompt(`งวด ${i+1}/${count} • ยอดที่สินเชื่อแจ้ง`,`0`);
    if(amt===null)return;
    const due=prompt(`งวด ${i+1}/${count} • วันครบกำหนด (YYYY-MM-DD)`,makeMonthlyDate(firstDue,i));
    if(due===null)return;
    round.installments.push({id:uid("inst"),no:i+1,dueDate:due,amount:Math.max(0,num(amt)),paid:false});
  }
...
กด OK = เพิ่มเข้า "เงินเข้า > เงินหมุน" อัตโนมัติ`);
  if(receivedNow){
    const incomeId=uid("inc");
    state.incomes.push({
      id:incomeId,name:`${debt.name} • ${loanRoundLabel(round)}`,amount:borrowed,date:startDate,
      kind:"rotation",note:`เงินกู้/เงินหมุน • ยอดชำระรวม ${money(round.totalRepayment)} • ต้นทุนรวมประมาณ ${money(round.totalInterest)}`,
      debtId:debt.id,loanRoundId:round.id,received:true
    });
  ...
  syncLoanRoundPayments(debt);
  saveState();renderAll();toast("เพิ่มรอบกู้ใหม่แล้ว");
}
...
function editLoanRound(debtId,roundId){
  ...
  const borrowed=prompt("ยอดเงินที่กด / ได้รับจริง",String(round.borrowedAmount||0));
  ...
  for(let idx=0;idx<round.installments.length;idx++){
    ...
    const amt=prompt(`งวด ${idx+1} • ยอดที่สินเชื่อแจ้ง`,String(inst.amount||0));
    if(amt===null)return;
  ...
  round.totalRepayment=round.installments.reduce((s,x)=>s+num(x.amount),0);
  round.totalInterest=Math.max(0,round.totalRepayment-round.borrowedAmount);
  round.totalFee=0;
  ...
  if(round.incomeId){
    ...
    if(inc){
      ...
      inc.note=`เงินกู้/เงินหมุน • ยอดชำระรวม ${money(round.totalRepayment)} • ต้นทุนรวมประมาณ ${money(round.totalInterest)}`;
    }
  }

  syncLoanRoundPayments(debt);
  saveState();renderAll();toast("แก้ไขรอบกู้แล้ว");
}
...
function ensureFlexibleLoanData(){
  state.debts.forEach(d=>{
    if(isSpecialLoanMode(d) && d.openingLoanRemaining==null && (!d.loanRounds || !d.loanRounds.length)){
      d.openingLoanBalance=Math.max(0,num(d.remaining));
      d.openingLoanRemaining=Math.max(0,num(d.remaining));
      state.payments.filter(p=>p.debtId===d.id && !p.loanRoundId).forEach(p=>p.openingLoanPayment=true);
    }
...
function renderAll(){
  ensureFlexibleLoanData();
  syncProductInstallmentSummaryPayments();
  renderDashboard();renderPayments();renderDebts();renderProductInstallments();renderIncome();renderRotations();renderForecast();renderRotationPlanner();renderSavedPlans();
}
